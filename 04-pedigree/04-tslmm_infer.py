...
if __name__ == "__main__":
    ...
    lmm = tslmm.TSLMM(ts, mu, pheno[subset], covariates[subset], phenotyped_individuals=subset, rng=rng, centre=True)
    lmm.fit_variance_components(method='ai', haseman_elston=True, verbose=True) 

    # get vc
    vc_path = args.out_prefix + f".{args.it}.vc.npy"
    vcs = np.zeros((2,2))
    vcs[0] = lmm.variance_components 
    vcs[1] = lmm._optimization_trajectory[0]
    np.save(vc_path, vcs)
